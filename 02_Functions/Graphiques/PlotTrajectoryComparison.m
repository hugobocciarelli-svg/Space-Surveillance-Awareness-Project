...
function PlotTrajectoryComparison(TimeT, Corrected_states, Z, meas_time, meas_datetime)
    time_meas_col = meas_datetime(:);
    
    % 1. Calculer les mesures en ECI 
    Meas_ECI = ECEFtoECI(Z, time_meas_col);
    ...
    for k = 1:3
        ...
        hold on;
        plot(meas_time, Meas_ECI(k, :), 'ro', 'MarkerSize', 4, 'MarkerFaceColor', 'r');
        grid on;
